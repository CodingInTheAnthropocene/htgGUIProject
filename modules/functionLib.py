...
def archiving(currentPath, archiveFolder):
    ...
    if currentPathType == "ShapeFile":
        ...
        shapefileNameWithoutExtension = path.splitext(shapefileNameWithExtension)[0]
        ...
        newDirectoryPath = (
            f"{archiveFolder}\\{shapefileNameWithoutExtension}{tenuresModifiedTime}"
        )
            ...
            for fileNameWExtension in files:
                ...
                if filename == shapefileNameWithoutExtension:
                    rename(
                        ...
                        path.join(
                            folderPath, f"{filename}{tenuresModifiedTime}{extension}"
                        ),
                    )
                    move(
    ...
    # NOTE, this needs to be refined
        ...
        make_archive(
            f"{archiveFolder}\\{fileName}{getFileCreatedDate(currentPath)}",
            "zip",
            currentGDB,
        )
...
def writeLog(settingsClass, currentPath, archiveStatus):
    ...
    archiveFolder = (
        "Archiving Error" if archiveStatus == False else settingsClass.archiveFolder
    )
...
def crownTenuresGeoprocessing(rawPath):
    ...
    tenuresSOIIntersect = arcpy.Intersect_analysis(
        [tenuresCopy, universalSettings.soiPath],
    ...
    return getPath(crownTenuresProcessedPath)
...
def crownTenuresProcess():
    ...
    rawPath = catalogueWarehouseDownload(
        crownTenuresSettings.downloadFolder,
        crownTenuresSettings.jsonPayload,
        crownTenuresSettings.fileName,
    )
    ...
    writeLog(crownTenuresSettings, processedFile)
...
def forestHarvestingAuthorityGeoprocessing(
    rawForestHarvestingAuthorityPath,
    downloadFolder,
    fileName,
    htgLandsPath,
    arcgisWorkspaceFolder,
):
    ...
    rawName = path.splitext(arcpy.Describe(rawForestHarvestingAuthorityPath).name)[0]
    ...
    htgLandsCopy = arcpy.CopyFeatures_management(htgLandsPath, "htglandsCopy")
...
def forestHarvestingAuthorityProcess():
    archiving(
        forestHarvestingAuthoritySettings.currentPath,
        forestHarvestingAuthoritySettings.archiveFolder,
    )
    forestHarvestingAuthorityGeoprocessing(
        catalogueWarehouseDownload(
            forestHarvestingAuthoritySettings.downloadFolder,
            forestHarvestingAuthoritySettings.jsonPayload,
            forestHarvestingAuthoritySettings.fileName,
        ),
        forestHarvestingAuthoritySettings.downloadFolder,
        forestHarvestingAuthoritySettings.fileName,
        universalSettings.htgLandsPath,
        forestHarvestingAuthoritySettings.arcgisWorkspaceFolder,
    )
...
def forestManagedLicenceGeoprocessing(
    rawForestManagedLicence,
    downloadFolder,
    fileName,
    htgLandsPath,
    arcgisWorkspaceFolder,
    ...
    # get name of Raw shape file
    rawName = path.splitext(arcpy.Describe(rawForestManagedLicence).name)[0]

    # create temporary GDB and copy raw shape file to it
    arcpy.CreateFileGDB_management(downloadFolder, "temp.gdb")
    tempGdbPath = f"{downloadFolder}\\temp.gdb"

    arcpy.FeatureClassToGeodatabase_conversion(rawForestManagedLicence, tempGdbPath)
    ...
    forestManagedLicenceProcessedPath = arcpy.Intersect_analysis(
        [forestManagedLicenceCopy, htgLandsCopy], fileName, join_attributes="NO_FID"
    )
...
def forestManagedLicenceProcess():
    archiving(
        forestManagedLicenceSettings.currentPath,
        forestManagedLicenceSettings.archiveFolder,
    )
    forestManagedLicenceGeoprocessing(
        catalogueWarehouseDownload(
            forestManagedLicenceSettings.downloadFolder,
            forestManagedLicenceSettings.jsonPayload,
            forestManagedLicenceSettings.fileName,
        ),
        forestManagedLicenceSettings.downloadFolder,
        forestManagedLicenceSettings.fileName,
        universalSettings.htgLandsPath,
        forestManagedLicenceSettings.arcgisWorkspaceFolder,
    )
...
def harvestedAreasGeoprocessing(
    rawHarvestedAreas, downloadFolder, fileName, htgLandsPath, arcgisWorkspaceFolder
):
    ...
    # get name of Raw shape file
    rawName = path.splitext(arcpy.Describe(rawHarvestedAreas).name)[0]

    # create temporary GDB and copy raw shape file to it
    arcpy.CreateFileGDB_management(downloadFolder, "temp.gdb")
    tempGdbPath = f"{downloadFolder}\\temp.gdb"

    arcpy.FeatureClassToGeodatabase_conversion(rawHarvestedAreas, tempGdbPath)
    ...
    harvestedAreasProcessedPath = arcpy.Intersect_analysis(
        [harvestedAreasCopy, htgLandsCopy], fileName, join_attributes="NO_FID"
    )
...
def harvestedAreasProcess():
    ...
    harvestedAreasGeoprocessing(
        catalogueWarehouseDownload(
            harvestedAreasSettings.jsonPayload,
            harvestedAreasSettings.downloadFolder,
            harvestedAreasSettings.fileName,
        ),
        harvestedAreasSettings.downloadFolder,
        harvestedAreasSettings.fileName,
        universalSettings.htgLandsPath,
        harvestedAreasSettings.arcgisWorkspaceFolder,
    )
...
def parcelMapBCGeoprocessing(rawPath):
    ...
    newGDB = arcpy.CreateFileGDB_management(
        parcelMapBCSettings.downloadFolder, "parcelMapContainer"
    )
    ...
    parcelMapProcessedPath = arcpy.Intersect_analysis(
        [parcelMapCopy, universalSettings.soiPath], parcelMapBCSettings.fileName
    )
...
def parcelMapBCProcess():
    ...
    parcelMapBCGeoprocessing(
        catalogueWarehouseDownload(
            parcelMapBCSettings.downloadFolder,
            parcelMapBCSettings.jsonPayload,
            parcelMapBCSettings.fileName,
        )
    )
...
def digitalRoadAtlasGeoprocessing(rawPath):
    ...
    roadAtlasIntersect = arcpy.Intersect_analysis(
        [roadAtlasDisolve, universalSettings.soiPath],
        digitalRoadAtlasSettings.fileName,
        "NO_FID",
    )
...
def digitalRoadAtlasProcess():
    try:
        archiving(
            digitalRoadAtlasSettings.currentPath, digitalRoadAtlasSettings.archiveFolder
        )
    except:
        print("Can't Archive, Check file path")

    digitalRoadAtlasGeoprocessing(
        (
            catalogueWarehouseDownload(
                digitalRoadAtlasSettings.downloadFolder,
                digitalRoadAtlasSettings.jsonPayload,
                digitalRoadAtlasSettings.fileName,
            )
        )
    )
...
def alcAlrPolygonsGeoprocessing(rawPath):
    ...
    landsCopy = arcpy.CopyFeatures_management(
        universalSettings.htgLandsPath,
        f"{path.split(universalSettings.htgLandsPath)[0]}\\tempLandsCopy",
    )
    ...
    alcAlrProcessed = arcpy.Intersect_analysis(
        [alcAlrCopy, landsCopy],
        alcAlrPolygonsSettings.fileName,
        join_attributes="NO_FID",
    )
...
def alcAlrPolygonsProcess():
    try:
        archiving(
            alcAlrPolygonsSettings.currentPath, alcAlrPolygonsSettings.archiveFolder
        )
    except:
        print("Can't Archive, Check file path")

    alcAlrPolygonsGeoprocessing(
        catalogueWarehouseDownload(
            alcAlrPolygonsSettings.downloadFolder,
            alcAlrPolygonsSettings.jsonPayload,
            alcAlrPolygonsSettings.fileName,
        )
    )
...
def environmentalRemediationSitesGeoprocessing(rawPath):
    ...
    remediationProcessed = arcpy.Intersect_analysis(
        [landsCopy, sitesCopy],
        environmentalRemediationSitesSettings.fileName,
        join_attributes="NO_FID",
    )
...
def environmentalRemediationSitesProcess():
    try:
        archiving(
            environmentalRemediationSitesSettings.currentPath,
            environmentalRemediationSitesSettings.archiveFolder,
        )

    except:
        print("Can't Archive, Check file path")

    environmentalRemediationSitesGeoprocessing(
        catalogueWarehouseDownload(
            environmentalRemediationSitesSettings.downloadFolder,
            environmentalRemediationSitesSettings.jsonPayload,
            environmentalRemediationSitesSettings.fileName,
        )
    )
